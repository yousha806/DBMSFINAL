...
class KMeansClustering:
    ...
    def m_step(self, data, cluster_assgn):
        ...
        for i in range(0,self.n_cluster):
          d[i]=[]
          # d[i].append(self.centroids[i])
        
        for i in range(0,len(data)):
          d[cluster_assgn[i]].append(data[i])
        new_bois=[]
        for j in d:
          
          avg=np.mean(d[j],axis=0,dtype=np.float64)
          
          new_bois.append(avg)
        self.centroids=(new_bois)
        return self
        pass
    ...
    def evaluate(self, data,cluster_assign):
        ...
        for i in range(0,len(data)):
          d[cluster_assign[i]].append(data[i])
        for key,listi in d.items():
          for i in listi:
