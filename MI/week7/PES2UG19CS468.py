...
class AdaBoost:
    ...
    def stump_error(self, y, y_pred, sample_weights):
        ...
        """
        M=np.where(y_pred!=y,1,0)
        error=np.sum(sample_weights*M)/(np.sum(sample_weights))
        return error
    def compute_alpha(self, error):
        ...
        eps = 1e-9
        # TODO
        alpha1=(0.5)*np.log((1-error)/(error+eps))
        return alpha1
    ...
    def update_weights(self, y, y_pred, sample_weights, alpha):
        ...
        # TODO
        var1=self.stump_error(y,y_pred,sample_weights)
        if(var1==0):
            return(sample_weights)*np.exp(alpha*(np.not_equal(y,y_pred)).astype(int))
        var2=1-var1
        for i in range(len(sample_weights)):
            if(y[i]==y_pred[i]):
                sample_weights[i]=sample_weights[i]/(2*var2)
            else:
                sample_weights[i]=sample_weights[i]/(2*var1)
    ...
    def predict(self, X):
        ...
        """
        # TODO
        predictions=np.array([self.stumps[j].predict(X) for j in range(self.n_stumps)])
